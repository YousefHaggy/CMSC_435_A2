...
def get_hot_deck_imputation(file):
	complete = pd.read_csv('dataset_complete.csv', nrows=10, na_values=['?']) 
	data = pd.read_csv(file, na_values=['?'], nrows=10 )
	closest_distances = {} 
	print(data)
	...
	for index, row in enumerate(data.itertuples(), 0):
		...
		for ci, col in enumerate(data.columns[:-1]):
			if not pd.isnull(data.loc[index,col]):
				continue
			i = 0
			while pd.isnull(closest_distances[index][i]["row"][ci]):
				i+=1
			data.loc[index,col] = closest_distances[index][i]["row"][ci]
	# Get MAE
